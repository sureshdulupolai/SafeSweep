...
export default function SuccessCard({ isOpen, onClose }) {
        ...
        >
          ...
          <div className="bg-brand-green/10 border-b border-brand-green/20 p-5 flex items-center justify-between relative overflow-hidden">
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-100 tracking-tight">Operation Successful</h3>
                <p className="text-xs text-brand-green/80 mt-0.5 font-medium flex items-center gap-1.5">
                  <ShieldCheck className="h-3.5 w-3.5" />
                  {deletedEmptyFolders.length} Empty Folders Permanently Eradicated
                </p>
              </div>
            </div>
          ...
          <div className="bg-brand-darkest/80 border-b border-brand-border p-5 grid grid-cols-3 gap-4">
             <div className="flex flex-col items-center justify-center p-3 bg-brand-card/40 rounded-xl border border-brand-border/50 shadow-inner">
                <span className="text-2xl font-black text-white">{deletedEmptyFolders.length}</span>
                <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mt-1">Folders Deleted</span>
             </div>
             <div className="flex flex-col items-center justify-center p-3 bg-brand-card/40 rounded-xl border border-brand-border/50 shadow-inner">
             ...
             <div className="flex flex-col items-center justify-center p-3 bg-brand-card/40 rounded-xl border border-brand-border/50 shadow-inner">
                ...
                <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mt-1">Storage Freed</span>
             </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-brand-darkest relative">
            <div className="mb-4">
               <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider mb-1">Destruction Log</h4>
               <p className="text-[11px] text-gray-500">The following directories have been cryptographically unlinked from the filesystem and permanently destroyed.</p>
            </div>
            ...
            <div className="border border-brand-border rounded-xl overflow-hidden bg-brand-card/30 shadow-inner">
              <div className="max-h-[40vh] overflow-y-auto p-2 space-y-1">
                {deletedEmptyFolders.length === 0 ? (
                  <div className="p-4 text-center text-xs text-gray-500">
                    No folders were deleted.
                  </div>
                ) : (
                  deletedEmptyFolders.map((path, idx) => (
                    <motion.div 
