...
import { Code, Search, ArrowRight, Save, Trash2, Folder, CheckCircle, Loader2, Download, Terminal } from 'lucide-react';
...
export default function DevCleaner() {
  const { 
    step, isScanning, isAnalyzing, devCaches, 
  ...
  const handleSelectDirectory = async (setter) => {
    if (window.api && window.api.selectDirectory) {
      ...
      // Basic fallback
      alert("Folder picker requires Electron Desktop App.");
    }
  };

  const handleScan = () => {
    scanCaches('C:\\', selectedLanguage);
  };
